class SmartCollection
  ...
  # This is used to prevent rbuf fill read timeout error due to huge json size
  ...
  def most_chatted
    ...
    #      ActiveRecord Query (without Limit)
    result = Product.joins(:chats,:score).
      select("products.*, COUNT(*) AS chat_init_count").
      where("message_type in (?) and message_date > ? and score > ?",MESSAGE_TYPE,@from_date,MIN_SCORE).
      group("products.id").
  ...
  def large_products_unlink_collection(collection_name)
    ...
    collection_ids.each do |collection_id|
      ...
      large_product_ids = (get_product_ids_by_cid(collection_id)).each_slice(MAX_PRODUCTS_LIMIT).to_a

      large_product_ids.each do |product_ids|
        puts
        p " ======================= Unlinking #{product_ids.size} Products from #{collection_name} ======================"
        puts
        hsh[:productIds] = product_ids
        write_api("delete",'collections/unlink-products-collection',hsh.to_json,"response","products")
      end
  ...
  def large_products_link_collection(collection_name)
    ...
    large_product_ids  = (send(method_name).map &:id).each_slice(MAX_PRODUCTS_LIMIT).to_a
    ...
    large_product_ids.each do |product_ids|
      puts
      p " ======================= Linking #{product_ids.size} Products To #{collection_name} ======================"
      puts
