class SmartCollections
 ...
 q=Chat.where("from_user = 31478 and message_date > ? and message_type = ?",@from_date,MESSAGE_TYPE)
 ...
 cc=ut.size
    ...
    result = Product.find_by_sql("select * from treasureProduct.products P
                     INNER JOIN (select product_id,count(DISTINCT signature) as chat_init_count 
                                 from (select product_id, case when from_user < to_user 
                                                           then concat(from_user, '#', to_user) 
                                                           else concat(to_user, '#', from_user) 
                                                          end as signature 
                                        from shopo_chats.chats
                                        where message_date > '#{@from_date}') t1                                
                                group by product_id
      ...
      #   where("chats.message_date > ? and score > ? ",@from_date,SCORE).
      #   group('products.id').
      #   order("chat_init_count DESC")
  
    
    my = result.map{|p| [p.id,p.chats.map{|chats| case when chats.from_user < chats.to_user 
                         then chats.from_user.to_s + "#" +  chats.to_user.to_s 
                        else chats.to_user.to_s + "#" + chats.from_user.to_s 
                   end if chats.message_date > @from_date}.compact.uniq.size]}
                   
    # my1 = result1.map{|p| [p.id,p.chat.map{|chats| case when chats.from_user < chats.to_user 
    #                      then chats.from_user.to_s + "#" +  chats.to_user.to_s 
    #                     else chats.to_user.to_s + "#" + chats.from_user.to_s 
    #                end }.uniq.size]}

    mybad = result.map { |p| [p.id,p.chat_init_count] }

    # mybad1 = result1.map { |p| [p.id,p.chat_init_count] }
   
    a = Product.joins(:chat).where("chats.from_user = 31478")
